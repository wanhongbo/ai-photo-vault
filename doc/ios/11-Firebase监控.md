...
- XcodeGen 添加 Crashlytics dSYM post-build script；缺少配置文件或 SwiftPM checkout 时跳过，不阻断本地构建。
...
2. 下载 `GoogleService-Info.plist`，放到 `ios/LumaNox/GoogleService-Info.plist`，确保 Xcode target 资源中包含该文件。
