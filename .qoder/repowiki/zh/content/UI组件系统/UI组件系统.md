...
- 重构HomeScreen、AlbumScreen、VaultSearchScreen的UI架构，全面采用AppTopBar组件
- 引入VaultEmptyActionButton组件，增强空状态操作按钮的一致性和用户体验
- 改进PressFeedback系统，新增extraHighlight参数和增强的视觉反馈效果
- 增强整体交互反馈机制的一致性和用户体验
...
- 反馈系统：PressFeedback提供按压缩放与透明度动画，支持extraHighlight参数；ThrottledClick提供点击节流，避免重复触发。
...
## 架构总览
Compose UI通过XpxVaultTheme集中注入颜色、排版与形状，业务屏幕以组合方式复用AppButton、AppDialog、AppTopBar与反馈修饰符，导航由MainActivity统一管理，屏幕间通过路由切换与参数传递解耦。
