...
标准媒体 Grid 列表用于所有以保险箱媒体为主体的内容池，包括相册详情、最近列表、搜索结果、回收站、AI 清理候选、AI 分类详情、AI 敏感候选与隐私打码媒体选择。

| 项 | 标准 |
|---|---|
| 页面外壳 | 二级媒体列表优先使用统一返回栏 + 滚动内容结构；与相册详情保持一致 |
| Grid 容器 | 宽度为屏幕可用宽度减去左右 16pt；背景 `sectionBg`，圆角 20pt，描边 `strokeStrong` |
| 容器内边距 | 16pt |
| 列数 | iPhone 固定 3 列；iPad 后续可扩展为 4-6 列 |
| 单元格 | 固定宽度计算，1:1 方形，不随内容数量、父级布局或文案变化拉伸 |
| 间距 | 列间距 8pt，行间距 8pt |
| 缩略图 | 真实解密缩略图，`.fill` 裁切，圆角 10-12pt，1pt 描边 |
| 视频标记 | 右下角播放图标或时长徽标 |
| 内容约束 | Grid 卡片内部只放媒体单元格或导入 tile；标题、计数、筛选、说明放在卡片外 |
| 空态 | 不显示空 Grid；使用标准空态卡片并给出导入、扫描或返回路径 |
| 交互 | 点击缩略图进入对应查看/编辑/打码页面；触摸目标不小于当前单元格尺寸 |

实现验收：SwiftUI 应优先复用 `VaultMediaGridCard` / `VaultMediaThumbnailView`。如果某个页面必须自定义 Grid，应在 Pen 和代码中保持上述列数、宽度、间距、圆角、描边和真实缩略图规则。
